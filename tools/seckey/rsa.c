...
#endif
	while(1) {
		//if (DEBUG) {DEBUG_N++; printf("DEBUG_N=%d\n", DEBUG_N);}
...
#define SMALL_A_B 15
		...
		//if (DEBUG_N == 15) printf ("a_d=%lx, b_d=%lx, sig_a_b=%d\n", a_d, b_d,sig_a_b);
...
//cli
  invalid = (len != d_len || blen < MIN_D_BIT ||
	     !bi_lt(d_len, data, nn_len, nn));
	return invalid;
}
...
#else
int do_rsa(int inlen, int *input, int klen, int *key, int nlen, int *n, int *output)
{
	int i=0;
	int r = bi_mon (inlen, input, klen, key, nlen, n, output);
	printf("length == %d\n",r);
