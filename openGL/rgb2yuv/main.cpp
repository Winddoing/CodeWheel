...
static int CreateDmaBufferFromTexture(int DrmFD,
		EGLDisplay Display,
		EGLContext Context,
		GLuint Texture,
		EGLImageKHR &Image)
...
// Not used
static EGLImageKHR CreateTextureFromFd(EGLDisplay Display,
		int fd,
		int Width,
		int Height,
		int Stride,
...
static GLuint PrepareComputeProgram()
{
...
static unsigned char *PrepareBMPFile(char *FileName)
{
...
static void PrepareTextures(int DrmFd, EGLDisplay Display, EGLContext Context, unsigned char *PixelData)
{
	// Input texture (RGBA)
...
static void ExecuteTest(GLuint ComputeHandle)
{
...
static int rgbatonv12(uint8_t *rgba_pixel_data, uint8_t *nv12_pixel_data)
{
	bool res;
	...
	PrepareTextures(fd, eglDpy, coreContext, rgba_pixel_data);
	GLuint computeHandle = PrepareComputeProgram();

	ExecuteTest(computeHandle);

	// Read NV12 pixel data
	GLuint fbo;
	...
	for (unsigned int i = 0; i < TextureWidth * (TextureHeight + (TextureHeight / 2)) * 4; i += 4) {
		nv12_pixel_data[k] = rgbaPixels[i];
		k++;
	}
...
int main(int argc, char *argv[])
{
	...
	rgbatonv12(pixelData, nv12ImageBuffer);
