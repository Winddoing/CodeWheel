...
struct resource *ress1 ,*ress2,*ress3,*ress4,*ress5,*ress6;
...
static int __devinit gec_key_probe(struct platform_device *pdev)
{
	...
	ress6 = platform_get_resource(pdev,IORESOURCE_IRQ,0);
	...
	*gpj2d = *gpj2d  &  ~(0xFF << 0);
	...
	//ret = request_irq(ress6->start, gec_handler, IRQF_DISABLED | IRQF_TRIGGER_FALLING, ress6->name, &button[i].key);


	gec_button = pdev->dev.platform_data;

	for(i=0;gec_button[i].irq > IRQ_EINT(15) && gec_button[i].irq < IRQ_EINT(27) ;i++)
	ret = request_irq(gec_button[i].irq, gec_handler, IRQF_DISABLED | IRQF_TRIGGER_FALLING, gec_button[i].name, &gec_button[i].key);
...
static int __devexit gec_key_remove(struct platform_device *pdev)
{
	...
	*gpj2d = *gpj2d | (0xFF << 0);
	...
	for(i=0;gec_button[i].irq > IRQ_EINT(15) && gec_button[i].irq < IRQ_EINT(27) ;i++)
	free_irq(gec_button[i].irq, &gec_button[i].key);
