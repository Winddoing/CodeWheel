...
static void dump_desc(char *title,struct dma_desc *desc)
{
	printk("=========== %s =========== \n",title);
	printk("dcm:0x%08lx\n",desc->dcm);
...
static void build_desc(unsigned char *desc,int burst,int len)
{
	...
	struct dma_desc *A = (struct dma_desc *)desc;
	struct dma_desc *B = (struct dma_desc *)(desc + len * 1);
	struct dma_desc *A1 = (struct dma_desc *)(desc + len * 2);
	struct dma_desc *B1 = (struct dma_desc *)(desc + len * 3);
...
static void start_dma_chans(struct cycle_raw_dma_test *dma)
{
	...
	list_for_each_entry(ch,&dma->top,list){
		struct jzdma_channel *dmac = to_jzdma_chan(ch->chan);
		...
		printk("ddrs:%p->0x%08lx\n",dmac->master->iomem + DDRS,BIT(dmac->id));
...
static void stop_dma_chans(struct cycle_raw_dma_test *dma)
{
	...
	list_for_each_entry(ch,&dma->top,list){
		...
		A = (struct dma_desc *)(ch->desc);
		B = (struct dma_desc *)(ch->desc + len * 1);
		A->dtc &= ~(0xff << 24);
		B->dtc &= ~(0xff << 24);
		dma_cache_sync(NULL,A,len*4,DMA_TO_DEVICE);
...
static void start_cycle_raw_dma(struct cycle_raw_dma_test *dma)
{
	...
	for(i = 0;i < ARRAY_SIZE(dma_test_burst_len);i++){
		...
		if (!chan) {
			printk("dma channel request failed! curid = %d\n",i);
			break;
		}
