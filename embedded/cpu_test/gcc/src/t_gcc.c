...
#if defined(USG) || defined(USGTIME)
#undef FLOAT
#include <sys/param.h>
...
#ifdef CORE_ONLY_JJOK
extern void show_string(unsigned char * x);
# define show_string(x) report_msg((x))
#else
# define show_string(x) printf("%s\n",x)
#endif
     ...
     char *name;
{
  ...
  if (name == 0 || !strcmp (name, "-"))
    ...
    finput = fopen (name, "r");
#ifdef USE_REAL_SOURCE
  if (finput == 0)
    pfatal_with_name (name);
#endif // USE_REAL_SOURCE
  ...
  init_obstacks ();
  show_string("----init_obstacks pass!----");
  init_tree_codes ();
  show_string("----init_tree_codes pass!----");
  init_lex ();
  show_string("----init_lex pass!----");
  ...
  init_rtl ();
  show_string("----init_rtl pass!----");
  init_emit_once (debug_info_level == DINFO_LEVEL_NORMAL
		  || debug_info_level == DINFO_LEVEL_VERBOSE);
  show_string("----init_emit_once pass!----");
  init_decl_processing ();
  show_string("----init_decl_processing pass!----");
  init_optabs ();
  show_string("----init_optabs pass!----");
  init_stmt ();
  show_string("----init_stmt pass!----");
  init_expmed ();
  show_string("----init_expmed pass!----");
  init_expr_once ();
  show_string("----init_expr_once pass!----");
  init_loop ();
  show_string("----init_loop pass!----");
  init_reload ();
  show_string("----init_reload pass!----");

  if (flag_caller_saves) {
    init_caller_save ();
    show_string("----init_caller_save pass!----");
  }
  ...
  if (flag_gen_aux_info)
	...
	show_string("----aux_info_file pass!----");
    }
  ...
  if (jump_opt_dump)
	...
	show_string("----jump_opt_dump pass!----");
    }
  ...
  if (local_reg_dump)
	...
	show_string("----local_reg_dump pass!----");	
    }
  ...
  if (global_reg_dump)
	...
	show_string("----global_reg_dump pass!----");	
    }
  ...
  if (dbr_sched_dump)
	...
	show_string("----dbr_sched_dump pass!----");		
    }
  ...
  if (stack_reg_dump)
	...
	show_string("----stack_regs_dump pass!----");		
  ...
  if (! name_specified && asm_file_name == 0)
	...
	show_string(asm_file_name);
	asm_out_file = fopen (asm_file_name, "w");
      }
      if (asm_out_file == 0)
	pfatal_with_name (asm_file_name);
    }
  show_string("----asm_out_file pass!----");	
#ifdef IO_BUFFER_SIZE
  ...
  if (yyparse () != 0)
    if (errorcount == 0) {
      ...
      show_string("----bison fail!----");      
    }
    else {
      show_string("----bison fatal err!----");      
      exit(1);
    }
  ...
  show_string("----Compilation is now finished except for writing symbol table!----");      
...
#elif defined VMS
	delete (aux_info_file_name);
#else
	DeleteFile(aux_info_file_name);
#endif
    }
