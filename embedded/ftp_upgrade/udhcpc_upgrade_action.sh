...
enter_upgrade()
{
	echo "Whether to upgrade through the network ..."

	local val=`echo $ip | awk -F '.' '{print $1$2$3}'`
	echo "ip=$ip, val=$val"
	if [ "$val" = "152173123" ]; then
		echo "Enter upgrade ..."
		/usr/bin/upgrade_start.sh
	else
		echo "wrong ip address, no upgrade."
	fi
