...
int TopologicalSort(GraphAdjList GL)
{
	...
	while( 0 != top )
	{
		...
		for( e=GL->adjList[gettop].firstedge; e; e=e->next )
		{
			...
			if( (etv[gettop] + e->adjvex) > etv[k] )
				etv[k] = etv[gettop] + e->adjvex;
		}
	}
...
void CriticalPath(GraphAdjList GL)
{
	...
	while( 0 != top2 )
	{
		...
		{
			k = e->adjvex;
			if( (ltv[k] - e->adjvex) < ltv[gettop] )
				ltv[gettop] = ltv[k] - e->adjvex;
		}
	}
	...
	for( j=0; j < GL->numVertexes; j++ )
	{
		for( e=GL->adjList[j].firstedge; e; e=e->next )
		{
			...
			if( ete == lte )
				printf("<v%d,v%d> length: %d , ", GL->adjList[j].data, GL->adjList[k].data, e->adjvex );
		}
	}
}
